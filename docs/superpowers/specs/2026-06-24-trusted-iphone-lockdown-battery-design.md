...
- libimobiledevice: native iOS device protocol stack, including lockdownd and Wi-Fi sync support. Licenses include LGPL-2.1 and GPL-2.0, so BatteryHub should avoid copying or bundling GPL command source without a separate packaging decision. <https://github.com/libimobiledevice/libimobiledevice>
...
- Do not add a row for an iPhone unless its UDID is in BatteryHub's trusted iPhone allowlist.
...
6. Settings also exposes a Forget action for trusted iPhones. Forgetting removes the BatteryHub allowlist entry and the local BatteryHub snapshot, but does not unpair the iPhone from macOS.

The first implementation may depend on external libimobiledevice CLI tools installed on the Mac. If tools are missing, BatteryHub reports a diagnostic row and points the user to install them. A separate packaging pass can decide whether to ship a signed helper or bundled libraries.
...
- Explanatory copy states that forgetting in BatteryHub does not remove the macOS trust pairing.
