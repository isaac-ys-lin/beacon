...
@main
final class BeaconMacApp: NSObject, NSApplicationDelegate, UNUserNotificationCenterDelegate {
    nonisolated(unsafe) private static var retainedDelegate: BeaconMacApp?

    private var model: BeaconModel?
    private var statusController: BeaconStatusController?
...
enum BeaconSystemSettingsActions {
...
enum BeaconStatusIconImage {
    static let designReferenceAssetName = BeaconSymbols.headerLogoAsset

    static func make(size: NSSize = NSSize(
        width: BeaconMenuBarMetrics.iconSide,
        height: BeaconMenuBarMetrics.iconSide
    )) -> NSImage {
        ...
        let image = beaconTemplateImage(size: statusIconSize)
            ...
            ?? NSImage(systemSymbolName: BeaconSymbols.app, accessibilityDescription: "Beacon")
            ?? NSImage(size: statusIconSize)

        image.size = statusIconSize
        image.isTemplate = true
        image.accessibilityDescription = "Beacon"
        return image
    }
    ...
    private static func beaconTemplateImage(size: NSSize) -> NSImage? {
        ...
        let image = NSImage(size: size, flipped: true) { _ in
            let scale = min(size.width, size.height)
                / BeaconStatusIconDrawingMetrics.canvasSide
                * BeaconStatusIconDrawingMetrics.artworkScale
            let origin = NSPoint(
                x: (size.width - BeaconStatusIconDrawingMetrics.canvasSide * scale) / 2,
                y: (size.height - BeaconStatusIconDrawingMetrics.canvasSide * scale) / 2
            )
            ...
            stem.lineWidth = BeaconStatusIconDrawingMetrics.stemLineWidth * scale
            ...
            drawArc(
                ...
                startDegrees: -16,
                endDegrees: -74,
                lineWidth: BeaconStatusIconDrawingMetrics.innerArcLineWidth * scale,
                scale: scale,
                origin: origin
            )
            drawArc(
                ...
                startDegrees: -12,
                endDegrees: -78,
                lineWidth: BeaconStatusIconDrawingMetrics.outerArcLineWidth * scale,
                scale: scale,
                origin: origin
            )

            let dotRadius = BeaconStatusIconDrawingMetrics.dotDiameter / 2
            let dotRect = NSRect(
                x: origin.x + (14 - dotRadius) * scale,
                y: origin.y + (15 - dotRadius) * scale,
                width: BeaconStatusIconDrawingMetrics.dotDiameter * scale,
                height: BeaconStatusIconDrawingMetrics.dotDiameter * scale
            )
...
private enum BeaconStatusIconDrawingMetrics {
