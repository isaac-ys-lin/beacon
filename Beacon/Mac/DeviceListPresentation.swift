...
public enum DeviceListItem: Equatable, Sendable {
    ...
    public var connectionState: ConnectionState {
        ...
        case .airPods(_, _, let components):
            return airPodsConnectionState(for: activeAirPodsComponents(components))
        }
    }
}
...
public struct DeviceDisplayPreferences: Equatable, Sendable {
    public static let pinnedDeviceIDsKey = "Beacon.pinnedDeviceIDs"
    public static let hiddenDeviceIDsKey = "Beacon.hiddenDeviceIDs"
...
public struct AirPodsAudioPreferences: Equatable, Sendable {
    public static let listeningModePrefix = "Beacon.airPodsAudio.listeningMode."
    public static let microphonePrefix = "Beacon.airPodsAudio.microphone."
...
private func airPodsConnectionState(for components: [AirPodsComponent]) -> ConnectionState {
    guard !components.isEmpty else { return .disconnected }
    if components.contains(where: { $0.connectionState == .connected }) {
        return .connected
    }
    if components.allSatisfy({ $0.connectionState == .disconnected }) {
        return .disconnected
    }
    return .unknown
}
